@extends('layouts.app')
@section('title', $product->name . ' - GlowUp Beauty')
    ...
    <div class="grid grid-cols-1 lg:grid-cols-2 gap-10">
        ...
        <div>
            <div class="bg-white rounded-2xl border border-gray-100 overflow-hidden shadow-sm">
                ...
                @else
                    <div class="aspect-square bg-gradient-to-br from-primary-100 to-accent-100 flex items-center justify-center">
                        <svg class="w-20 h-20 text-primary-300" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="1.5" d="M4 16l4.586-4.586a2 2 0 012.828 0L16 16m-2-2l1.586-1.586a2 2 0 012.828 0L20 14m-6-6h.01M6 20h12a2 2 0 002-2V6a2 2 0 00-2-2H6a2 2 0 00-2 2v12a2 2 0 002 2z"/></svg>
                    </div>
                @endif
            </div>
        ...
        <div>
            ...
            @if($product->brand)
                <p class="text-gray-500 mt-1">Brand: <span class="font-medium text-gray-700">{{ $product->brand }}</span></p>
            @endif
            ...
            <div class="flex items-center gap-3 mt-3">
                ...
                <span class="text-sm text-gray-500">{{ $product->average_rating }} ({{ $product->approvedReviews->count() }} review)</span>
            </div>
            ...
            <div class="mt-5 p-4 bg-primary-50/60 rounded-xl">
                @if($product->discount_price)
                    <div class="flex items-center gap-3">
                        <span class="text-3xl font-extrabold text-primary-600">Rp {{ number_format($product->discount_price, 0, ',', '.') }}</span>
                        <span class="text-lg text-gray-400 line-through">Rp {{ number_format($product->price, 0, ',', '.') }}</span>
                        <span class="bg-red-500 text-white text-xs font-bold px-2 py-0.5 rounded-full">-{{ $product->discount_percent }}%</span>
                    </div>
                @else
            ...
            <div class="mt-6 space-y-3">
                <form action="{{ route('cart.add') }}" method="POST" class="flex items-center gap-3">
                    ...
                    <div class="flex items-center border border-gray-200 rounded-xl overflow-hidden">
                        <button type="button" data-qty-minus class="px-3 py-3 hover:bg-gray-100 text-gray-500 transition-colors">
                            <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M20 12H4"/></svg>
                        </button>
                        <input type="number" name="quantity" value="1" min="1" max="{{ $product->stock }}"
                               class="w-14 text-center border-0 text-sm font-semibold focus:ring-0">
                        <button type="button" data-qty-plus class="px-3 py-3 hover:bg-gray-100 text-gray-500 transition-colors">
                    ...
                    <button type="submit" class="flex-1 btn-primary text-white font-semibold py-3 px-6 rounded-xl text-sm flex items-center justify-center gap-2"
                ...
                <form action="{{ route('wishlist.toggle') }}" method="POST">
                    @csrf
                    <input type="hidden" name="product_id" value="{{ $product->id }}">
                    <button type="submit" class="w-full py-3 px-6 rounded-xl text-sm font-semibold border-2 border-primary-200 text-primary-600 hover:bg-primary-50 transition-colors flex items-center justify-center gap-2">
            ...
            @else
            <div class="mt-6">
                <a href="{{ route('login') }}" class="block w-full btn-primary text-white font-semibold py-3 px-6 rounded-xl text-sm text-center">
                    Login untuk Membeli
                </a>
            </div>
            @endauth
            ...
            <form action="{{ route('reviews.store') }}" method="POST" class="space-y-4">
                ...
                <div>
                    ...
                    <div class="star-rating flex items-center gap-1">
                        ...
                        @for($i = 1; $i <= 5; $i++)
                        <svg data-value="{{ $i }}" class="star w-8 h-8 cursor-pointer {{ $i <= 5 ? 'active text-amber-400' : 'text-gray-300' }}" fill="currentColor" viewBox="0 0 20 20">
                            <path d="M9.049 2.927c.3-.921 1.603-.921 1.902 0l1.07 3.292a1 1 0 00.95.69h3.462c.969 0 1.371 1.24.588 1.81l-2.8 2.034a1 1 0 00-.364 1.118l1.07 3.292c.3.921-.755 1.688-1.54 1.118l-2.8-2.034a1 1 0 00-1.175 0l-2.8 2.034c-.784.57-1.838-.197-1.539-1.118l1.07-3.292a1 1 0 00-.364-1.118L2.98 8.72c-.783-.57-.38-1.81.588-1.81h3.461a1 1 0 00.951-.69l1.07-3.292z"/>
                        </svg>
                        @endfor
                    </div>
                ...
                <div>
                    ...
                    <textarea id="comment" name="comment" rows="3" placeholder="Bagikan pengalaman kamu dengan produk ini..."
        ...
        <div class="space-y-4">
            ...
            <div class="bg-white rounded-xl border border-gray-100 p-5">
                <div class="flex items-center justify-between mb-2">
                    <div class="flex items-center gap-3">
                        <div class="w-9 h-9 rounded-full bg-gradient-to-br from-primary-400 to-accent-400 flex items-center justify-center text-white text-sm font-bold">
