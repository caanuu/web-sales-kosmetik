@extends('layouts.app')
@section('title', 'Produk - Sahata Cosmetic')
                    ...
                    <div class="bg-white rounded-2xl shadow-sm border border-gray-100 p-5 space-y-6">
                            ...
                            <div class="space-y-2">
                                <label class="flex items-center gap-2 cursor-pointer">
                                    <input type="radio" name="category" value="" {{ !request('category') ? 'checked' : '' }}
                                        class="text-primary-500 focus:ring-primary-400" onchange="this.form.submit()">
                        ...
                        <div>
                            ...
                            <div class="flex gap-2">
                                <input type="number" name="min_price" value="{{ request('min_price') }}" placeholder="Min"
                                    class="w-full px-3 py-2 rounded-lg border border-gray-200 text-sm focus:border-primary-400 focus:ring-1 focus:ring-primary-200">
                        ...
                        @if($brands->count() > 0)
                            ...
                            <div>
                                ...
                                <select name="brand"
                                    class="w-full px-3 py-2 rounded-lg border border-gray-200 text-sm focus:border-primary-400"
                        ...
                        @if(request()->hasAny(['category', 'min_price', 'max_price', 'brand', 'search']))
                            <a href="{{ route('products.index') }}"
                                class="block text-center text-sm text-gray-500 hover:text-primary-500">Reset Filter</a>
                        @endif
                    </div>
                </form>
            </aside>
            ...
            <div class="flex-1">
                ...
                <div class="flex flex-col sm:flex-row items-start sm:items-center justify-between mb-6 gap-3">
                    ...
                    <form action="{{ route('products.index') }}" method="GET" class="flex items-center gap-2">
                        ...
                        <select name="sort" onchange="this.form.submit()"
                            class="px-3 py-2 rounded-lg border border-gray-200 text-sm focus:border-primary-400 bg-white">
                ...
                @else
                    <div class="text-center py-20">
                        ...
                        <a href="{{ route('products.index') }}"
                            class="inline-block mt-4 text-sm text-primary-600 font-semibold hover:text-primary-700">Reset
                            Pencarian</a>
                    </div>
                @endif
            </div>
