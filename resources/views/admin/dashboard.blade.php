        ...
        <div class="bg-white rounded-2xl p-6 shadow-sm border border-gray-100">
            <div class="flex items-center justify-between">
                <div>
                    ...
                    <p class="text-2xl font-extrabold text-gray-800 mt-1">Rp
        ...
        <div class="bg-white rounded-2xl p-6 shadow-sm border border-gray-100">
            <div class="flex items-center justify-between">
                <div>
                    ...
                    <p class="text-3xl font-extrabold text-purple-600 mt-1">{{ number_format($stats['pending_reviews']) }}
                    </p>
                </div>
                            ...
                            <tr class="border-b border-gray-50 hover:bg-gray-50">
                                <td class="py-3 px-2">
                                    <a href="{{ route('admin.orders.show', $order) }}"
                                        class="text-primary-600 font-medium hover:underline">{{ $order->order_number }}</a>
                                </td>
                                <td class="py-3 px-2 text-gray-700">{{ $order->user->name }}</td>
                                <td class="py-3 px-2 font-medium text-gray-800">Rp {{ number_format($order->total, 0, ',', '.') }}
                                </td>
                                <td class="py-3 px-2">
                                    @php $c = ['pending' => 'yellow', 'processing' => 'blue', 'shipped' => 'purple', 'completed' => 'green', 'cancelled' => 'red'][$order->status] ?? 'gray'; @endphp
                                    <span
                                        class="text-xs font-semibold px-2.5 py-1 rounded-full bg-{{ $c }}-100 text-{{ $c }}-700">{{ $order->status_label }}</span>
                                </td>
