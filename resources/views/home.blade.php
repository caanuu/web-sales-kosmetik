...
<section class="relative bg-gradient-to-br from-pink-100 via-primary-50 to-fuchsia-50 overflow-hidden">
    <div class="absolute inset-0 opacity-40">
        <div class="absolute top-10 left-10 w-96 h-96 bg-pink-300 rounded-full mix-blend-multiply filter blur-3xl animate-pulse"></div>
        <div class="absolute bottom-10 right-10 w-96 h-96 bg-purple-300 rounded-full mix-blend-multiply filter blur-3xl animate-pulse" style="animation-delay: 2s"></div>
    </div>

    <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-20 lg:py-28 relative">
        <div class="max-w-2xl">
            <span class="inline-block px-4 py-1.5 bg-white/70 backdrop-blur rounded-full text-xs font-semibold text-pink-600 mb-6 border border-pink-200">
                ✨ Koleksi Terbaru 2025
            </span>
            <h1 class="text-4xl lg:text-6xl font-extrabold text-gray-900 leading-tight">
                Temukan
                <span class="text-transparent bg-clip-text bg-gradient-to-r from-pink-500 to-purple-600">Kecantikan</span>
                Terbaik Untukmu
            </h1>
            ...
            <div class="mt-8 flex flex-wrap gap-3">
                <a href="{{ route('products.index') }}" class="btn-primary px-8 py-3.5 text-white font-semibold rounded-full text-sm inline-flex items-center gap-2 shadow-lg shadow-pink-500/30">
                    Belanja Sekarang
                    <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M17 8l4 4m0 0l-4 4m4-4H3"/></svg>
                </a>
                <a href="#categories" class="px-8 py-3.5 border-2 border-pink-300 text-pink-600 font-semibold rounded-full text-sm hover:bg-pink-50 transition-colors bg-white/50 backdrop-blur">
                    Lihat Kategori
                </a>
            </div>
            ...
            <div class="mt-10 flex flex-wrap items-center gap-6 text-sm text-gray-600 dark:text-gray-200 font-medium">
                <div class="flex items-center gap-2 bg-white/80 dark:bg-white/10 px-3 py-1.5 rounded-lg border border-pink-100 dark:border-white/10 backdrop-blur-sm">
                    <svg class="w-5 h-5 text-green-500" fill="currentColor" viewBox="0 0 20 20"><path fill-rule="evenodd" d="M10 18a8 8 0 100-16 8 8 0 000 16zm3.707-9.293a1 1 0 00-1.414-1.414L9 10.586 7.707 9.293a1 1 0 00-1.414 1.414l2 2a1 1 0 001.414 0l4-4z" clip-rule="evenodd"/></svg>
                    100% Original
                </div>
                <div class="flex items-center gap-2 bg-white/80 dark:bg-white/10 px-3 py-1.5 rounded-lg border border-pink-100 dark:border-white/10 backdrop-blur-sm">
                    <svg class="w-5 h-5 text-green-500" fill="currentColor" viewBox="0 0 20 20"><path d="M8 16.5a1.5 1.5 0 11-3 0 1.5 1.5 0 013 0zM15 16.5a1.5 1.5 0 11-3 0 1.5 1.5 0 013 0z"/><path d="M3 4a1 1 0 00-1 1v10a1 1 0 001 1h1.05a2.5 2.5 0 014.9 0H14a1 1 0 001-1v-2l-2-5H4V4H3z"/></svg>
...
@if(isset($banners) && $banners->count() > 0)
    <section class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 pb-16">
        <div class="space-y-8">
            ...
            <div class="bg-gradient-to-r from-primary-600 via-primary-500 to-accent-500 rounded-3xl overflow-hidden shadow-lg relative flex flex-col md:flex-row min-h-[300px]">
                <div class="p-10 md:p-12 text-left text-white relative z-10 flex-1 flex flex-col justify-center">
                    @if($banner->subtitle)
                        <span class="inline-block mb-3 text-pink-200 font-bold uppercase tracking-wider text-sm">{{ $banner->subtitle }}</span>
                    @endif
                    ...
                    @if($banner->button_text && $banner->button_link)
                        <div class="mt-8">
                            <a href="{{ url($banner->button_link) }}" class="inline-block bg-white text-primary-600 font-bold px-8 py-3.5 rounded-full text-sm hover:bg-pink-50 transition-colors shadow-md hover:shadow-xl transform hover:-translate-y-0.5 duration-200">
                                {{ $banner->button_text }}
                            </a>
                        </div>
                    @endif
                ...
                <div class="md:w-5/12 h-64 md:h-auto relative overflow-hidden">
                    <div class="absolute inset-0 bg-gradient-to-t md:bg-gradient-to-l from-transparent via-accent-500/20 to-accent-500 z-10"></div>
                    <img src="{{ asset('storage/' . $banner->image) }}" alt="{{ $banner->title }}" class="w-full h-full object-cover object-center absolute inset-0">
                </div>
                @else
...
@else
    {{-- Fallback Static CTA --}}
    <section class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 pb-16">
        <div class="bg-gradient-to-r from-primary-600 via-primary-500 to-accent-500 rounded-3xl p-10 lg:p-14 text-center text-white relative overflow-hidden shadow-lg">
            <div class="absolute inset-0 opacity-10 pointer-events-none">
                <div class="absolute -top-20 -left-20 w-60 h-60 bg-white rounded-full"></div>
                <div class="absolute -bottom-20 -right-20 w-80 h-80 bg-white rounded-full"></div>
            </div>
            <div class="relative z-10">
                ...
                @guest
                <a href="{{ route('register') }}" class="inline-block mt-6 bg-white text-primary-600 font-bold px-8 py-3.5 rounded-full text-sm hover:bg-pink-50 transition-colors shadow-lg hover:shadow-xl transform hover:-translate-y-0.5 duration-200">
