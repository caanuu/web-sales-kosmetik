@extends('layouts.app')
@section('title', 'Checkout - Sahata Cosmetic')

@section('content')
    <div class="max-w-5xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        <div class="flex items-center gap-4 mb-6">
            <a href="javascript:history.back()"
                class="inline-flex items-center gap-2 text-sm font-medium text-gray-500 hover:text-primary-600 transition-colors bg-white px-4 py-2 rounded-full border border-gray-100 shadow-sm hover:shadow-md">
        ...
        <form action="{{ route('checkout.store') }}" method="POST">
            ...
            <div class="grid grid-cols-1 lg:grid-cols-3 gap-8">
                ...
                <div class="lg:col-span-2 space-y-6">
                    <div class="bg-white rounded-2xl border border-gray-100 p-6 shadow-sm">
                        ...
                        <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                            ...
                            <div>
                                ...
                                <input type="text" name="recipient_phone" value="{{ old('recipient_phone', $user->phone) }}"
                                    required
                        ...
                        <div>
                            <label class="block text-sm font-medium text-gray-700 mb-1.5">Wilayah Pengiriman (Sumatera
                                Utara)</label>
                            <select name="shipping_area_id" id="shipping_area" required
                                class="w-full px-4 py-3 rounded-xl border border-gray-200 focus:border-primary-400 focus:ring-2 focus:ring-primary-200 text-sm @error('shipping_area_id') border-red-300 @enderror">
                    ...
                    <div class="mt-4">
                        <label class="block text-sm font-medium text-gray-700 mb-1.5">Alamat Lengkap Lengkap (Jalan, RT/RW,
                            Kecamatan)</label>
                ...
                <div class="bg-white rounded-2xl border border-gray-100 p-6 shadow-sm">
                    ...
                    <div class="space-y-3">
                        <label
                            class="flex items-center gap-3 p-4 rounded-xl border-2 cursor-pointer transition-colors has-[:checked]:border-primary-400 has-[:checked]:bg-primary-50 dark:has-[:checked]:bg-primary-900/30 border-gray-200 hover:border-gray-300">
                ...
                <div class="bg-white rounded-2xl border border-gray-100 p-6 shadow-sm sticky top-24">
                        ...
                        @foreach($carts as $cart)
                            <div class="flex items-center gap-3">
                                ...
                                <div class="flex-1 min-w-0">
                                    ...
                                    <p class="text-xs text-gray-400">{{ $cart->quantity }}x Rp
                                        {{ number_format($cart->product->effective_price, 0, ',', '.') }}</p>
                                </div>
                                <span class="text-xs font-semibold text-gray-700">Rp
                                    {{ number_format($cart->subtotal, 0, ',', '.') }}</span>
                            </div>
                        @endforeach
                    </div>
                    ...
                    <div class="space-y-2 text-sm">
                        ...
                        <div class="flex justify-between font-bold text-gray-800 text-base">
                            <span>Total</span>
                            <span class="text-primary-600" id="total_display" data-subtotal="{{ $subtotal }}">Rp
                                {{ number_format($subtotal, 0, ',', '.') }}</span>
                        </div>
                    </div>

                    <button type="submit"
                        class="w-full btn-primary text-white font-semibold py-3 rounded-xl text-sm mt-6 flex items-center justify-center gap-2">
